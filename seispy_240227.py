...
def magnitude_to_moment(magnitudes):
    """
    Covert moment magnitude to seismic moment
    """
    magnitudes = np.array(magnitudes)
    seismic_moments = 10**(1.5*(magnitudes + 10.7))
    return seismic_moments
...
def get_catalogue_extent(catalogue):
    """
    Returns the min/max of the Lon/Lat of an earthquake catalogue
    """
    return np.array([min(catalogue['LON']), max(catalogue['LON']), min(catalogue['LAT']), max(catalogue['LAT'])])
...
def select_mainshocks(earthquake_catalogue,
    ...
    for mainshock in earthquakes_above_magnitude_threshold.itertuples():
        ...
        if len(local_catalogue_1yr)>0:
            max_magnitude = max(local_catalogue_1yr['MAGNITUDE'])
            Mc = Mc_by_maximum_curvature(local_catalogue['MAGNITUDE'])
        ...
        results_dict = {'ID':mainshock.ID,
                        ...
                        '1yr_local_catalogue':len_local_catalogue,
                        'Mc':Mc,
...
def identify_foreshocks(mainshock_file,
        ...
        if plot == True:
            ...
            if len(sliding_window_counts) > 0:
                axs[model_plot].hist(sliding_window_counts, bins=range(min(sliding_window_counts)-1, max(sliding_window_counts)+1), color='yellow',
                        density=True, rwidth=1.0, alpha=0.3, align=align, label=method_dict['ESR_2'] + ': ' + str(round(sliding_window_probability,3)))
                ...
                axs[CDF_plot].plot(window_counts_sorted, window_counts_cdf, label=method_dict['ESR_2'], color='yellow', alpha=0.3)
    ...
    results_file = pd.merge(mainshock_file, results_file, on='ID', how='left')
